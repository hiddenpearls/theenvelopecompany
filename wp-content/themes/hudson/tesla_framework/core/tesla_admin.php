...
//=============================================END AUTOIMPORT DEMO CONTENT XML====================================================
  ...
  function tt_purchase_notice() {
    ...
    if( in_array( $this->tesla_security->username, array( 'tt_general_user','tt_other_marketplaces_user' ) ) ){
      ...
      if(!empty($api->subscriptions->{3}))
        $message = "Upgrade to Pro for only $20 in $dashboard_link.";
      else
  ...
  function tt_dismiss_notice_ajax(){
    set_transient( 'notice_dismissed' , true , 5 * DAY_IN_SECONDS );
  }
