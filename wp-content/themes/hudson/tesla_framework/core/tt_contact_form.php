...
if(!class_exists('TT_Contact_Form_Builder')){
	class TT_Contact_Form_Builder {
		...
		public static function load_scripts(){
			add_action( 'admin_enqueue_scripts',array('TT_Contact_Form_Builder', 'builder_scripts') , 99);
		}
		...
		public static function send_message_ajax(){
			if(!empty($_POST)){
				...
				if(!empty($form_data['receiver_email'])){	//hidden input field in the form markup
					$receiver_email = $form_data['receiver_email'];
					unset($form_data['receiver_email']);
				}elseif(!empty($form['receiver_email'])){	//option in Form builder
