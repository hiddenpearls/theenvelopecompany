		...
		public static function generate_custom_fields(){
			...
			foreach($slider_options as $slider_id => $slider){
				if(isset( $slider['icon'] ))
					$menu_icon = strpos($slider['icon'], 'ashicons') ? $slider['icon'] : TT_THEME_URI.'/theme_config/' . $slider['icon'];
				else
					$menu_icon = null;
				...
				$slider_url_rewrite = apply_filters( 'tt_cpt_rewrite_slug_' . $slider_id, $slider_url_rewrite );
