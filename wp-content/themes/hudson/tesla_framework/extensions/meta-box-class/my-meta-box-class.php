...
class AT_Meta_Box {
  ...
  public function __construct ( $meta_box ) {
    ...
    if (isset($meta_box['use_with_theme']))
      if ($meta_box['use_with_theme'] === true){
        $this->SelfPath = TT_FW . '/extensions/meta-box-class';
  ...
  public function load_scripts_styles() {
    ...
    if (in_array($typenow,$this->_meta_box['pages']) && $this->is_edit_page()){
      ...
      foreach (array('upload','color','date','time','datetime','code','select') as $type) {
  ...
  public function check_field_datetime() {
    ...
    if ( $this->has_field( 'datetime' ) && $this->is_edit_page() ) {
      $plugin_path = $this->SelfPath;
      // Enqueu JQuery UI, use proper version.
      wp_enqueue_style( 'at-datetime', $plugin_path . '/css/jquery.datetimepicker.css' );
      wp_enqueue_script( 'jquery-ui');
      wp_enqueue_script( 'at-datetime', $plugin_path . '/js/jquery.datetimepicker.js', array( 'jquery' ), null, true );
    }
  }
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px; min-height: 50px}
  ...
  public function show_field_datetime( $field, $meta ) {
    $meta = date('d-m-Y H:i',$meta);
    $this->show_field_begin( $field, $meta );
      echo "<input type='text'  ".( isset($field['style'])? "style='{$field['style']}' " : '' )." class='at-datetime".( isset($field['class'])? ' ' . $field['class'] : '' )."' name='{$field['id']}' id='{$field['id']}' value='{$meta}' size='30' />";
    $this->show_field_end( $field, $meta );
  }
  ...
  public function save_field_datetime( $post_id, $field, $old, $new ) {
    $name = $field['id'];
    delete_post_meta( $post_id, $name );
    if ( $new === '' || $new === array() ) 
      return;
  ...
  function Validate_upload_file_type($file) {
    if (isset($_POST['uploadeType']) && !empty($_POST['uploadeType']) && isset($_POST['uploadeType']) && $_POST['uploadeType'] == 'my_meta_box'){
      ...
      if (!in_array($ext, (array)$allowed)){
        $file['error'] = __("Sorry, you cannot upload this file type for this field.",'TeslaFramework');
        return $file;
      }
