...
//===========================AJAX SAVE OPTIONS======================================================================
  ...
  if($('.mapoptions').length){
    $('.mapoptions').each(function(){
      editors[$(this).attr('id')] = CodeMirror.fromTextArea(document.getElementById($(this).attr('id')), {
        lineNumbers: true,
        mode: "javascript",
        theme: "mdn-like",
        matchBrackets: true,
...
function map_editors_reinit(){
  if(editors){
    jQuery('.mapoptions').each(function(index,element){
      editors[jQuery(element).attr('id')].refresh();
    });
  }
}
function map_editors_save(){
  if(editors){
    jQuery('.mapoptions').each(function(index,element){
      editors[jQuery(element).attr('id')].save();
    });
  }
}
...
jQuery(function($) {
  ...
  //reiniting sliders and code editors when on different tab than first opened
  var code_editors_reinited = false
  $('a[data-toggle=tab]').on('click', function(e){
    if(code_editors_reinited === false){
      setTimeout(map_editors_reinit,100);
    }
  });
});