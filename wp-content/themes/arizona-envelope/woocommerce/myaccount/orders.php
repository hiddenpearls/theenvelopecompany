...
<?php if ( $has_orders ) : ?>
	...
	<table class="woocommerce-MyAccount-orders shop_table shop_table_responsive my_account_orders account-orders-table">
		<thead>
			<tr>
				<?php foreach ( azenv_wc_get_account_orders_columns() as $column_id => $column_name ) : ?>
		...
		<tbody>
			<?php foreach ( $customer_orders->orders as $customer_order ) :
				...
				//print_r($order);
				//echo "</pre>";
                // Store the order ID and get the PO Number for that specific order
                $order_retrieve = $order->id;
                global $wpdb;
                $results = $wpdb->get_results("SELECT `meta_value` FROM wp_postmeta WHERE `post_id`=$order_retrieve AND `meta_key`='po_reference_number'");
					...
					<?php foreach ( azenv_wc_get_account_orders_columns() as $column_id => $column_name ) : ?>
						<?php if( $column_name != 'Status' ){  ?>
							<td class="<?php echo esc_attr( $column_id ); ?>" data-title="<?php echo esc_attr( $column_name ); ?>">
                                ...
                                <?php elseif ( 'order-po' === $column_id ) : ?>
                                    <?php echo $results[0]->meta_value; ?>
                                    <?php //do_action( 'woocommerce_order_details_after_customer_details', $order ); ?>

								<?php elseif ( 'order-total' === $column_id ) : ?>
									<?php echo sprintf( _n( '%s for %s item', '%s for %s items', $item_count, 'woocommerce' ), $order->get_formatted_order_total(), $item_count*500 ); ?>
