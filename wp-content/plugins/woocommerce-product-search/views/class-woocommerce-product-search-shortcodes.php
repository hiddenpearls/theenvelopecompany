...
class WooCommerce_Product_Search_Shortcodes {
	...
	/**
	 * Shortcode handler, renders a product search form.
	 * 
	...
	public static function woocommerce_product_search( $atts = array(), $content = '' ) {
		...
		$atts = shortcode_atts(
			array(
				...
				'show_price'         => null,
				'placeholder'     => __( 'Search', WOO_PS_PLUGIN_DOMAIN ),
				'no_results'      => '',
				...
				'inhibit_enter' => 'no',
				'submit_button' => 'no',
				'submit_button_label' => __( 'Search', WOO_PS_PLUGIN_DOMAIN ),
				'navigable'     => 'yes',
				'auto_adjust'   => 'yes',
				'wpml'          => 'no'
			),
		...
		if ( $params['delay'] < WooCommerce_Product_Search::MIN_DELAY ) {
			$params['delay'] = WooCommrece_Product_Search::MIN_DELAY;
		}
		...
		// These are not taken into account as the default search includes them and
		// we should not interfere with that to avoid unexpected side-effects.
		...
		$output .= '</div>';
		$output .= '</form>';
		$output .= '</div>'; // .product-search-form

		$output .= sprintf( '<div id="%s" class="product-search-results">', $results_id );
		$output .= '</div>'; // .product-search-results

		$output .= '</div>'; // .product-search
		...
		if ( apply_filters( 'woocommerce_product_search_use_admin_ajax', WooCommerce_Product_Search::USE_ADMIN_AJAX_DEFAULT ) ) {
			// if used as alternative, persistent caching doesn't work with that
			$post_target_url = add_query_arg( $url_params , admin_url( 'admin-ajax.php' ) ); 
		} else {
			$post_target_url = add_query_arg( $url_params , WOO_PS_PLUGIN_URL . '/core/product-search.php' );
		}
		...
		$output .= sprintf(
			...
			esc_attr( $field_id ), // jQuery selector for the input field passed to productSearch()
			esc_attr( $search_id ), // container selector
			esc_attr( $search_id . ' div.product-search-results' ), // results container selector 			
			$post_target_url,
			$js_args,//!empty( $atts['no_results'] ) ? sprintf( '{no_results:"%s"}', esc_js( $atts['no_results'] ) ) : '{}',
			$params['delay'],
			$params['characters']
		);
