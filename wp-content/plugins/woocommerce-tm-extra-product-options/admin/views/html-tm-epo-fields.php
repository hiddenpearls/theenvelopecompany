...
?>
<div class="wrap">
	<h2><?php _e( 'TM Global Extra Product Options', 'woocommerce-tm-extra-product-options' );
...
foreach ( $bulk_counts as $message => $count ) {
	...
	if ( $message == 'trashed' && isset( $_REQUEST['ids'] ) ) {
		...
		if (isset($tm_nonce)){
			$messages[] = '<a href="' . esc_url( wp_nonce_url( "edit.php?post_type=product&page=".TM_EPO_GLOBAL_POST_TYPE_PAGE_HOOK."&doaction=undo&action=untrash&ids=$ids".$tm_bulk, $tm_nonce ) ) . '">' . __('Undo', 'woocommerce-tm-extra-product-options') . '</a>';
		}
	}
}
...
unset( $messages );
if ( $error_messages )
	echo '<div id="message" class="error"><p>' . join( ' ', $error_messages ) . '</p></div>';
...
<input type="hidden" name="page" class="page_page" value="<?php echo TM_EPO_GLOBAL_POST_TYPE_PAGE_HOOK; ?>" />
