								...
								if ( ! empty($missing_ids) && is_array($missing_ids) ){																											
									...
									foreach ($missing_ids_arr as $key => $missingPostRecords) {
										...
										if ( ! empty($missingPostRecords) ){ 
											...
											return array(
												'status'     => 200,
												'message'    => sprintf(__('Deleted missing records %s for import #%s', 'wp_all_import_plugin'), count($missing_ids), $this->id)
											);							
										}										
									}									
								}
	...
	public function process($xml, $logger = NULL, $chunk = false, $is_cron = false, $xpath_prefix = '', $loop = 0) {
		...
		try { 						
			...
			if ( in_array($this->options['custom_type'], array('taxonomies')) ){
                ...
                if ( ! empty($this->options['taxonomy_parent']) ){
                ...
                $taxonomy_slug = array();
                if ( 'xpath' == $this->options['taxonomy_slug'] && ! empty($this->options['taxonomy_slug_xpath']) ){
            ...
            if ( ! empty($post_taxonomies) && ! in_array($this->options['custom_type'], array('import_users', 'taxonomies')) ):
