...
class backupbuddy_migrateDB {
	...
	function bruteforceTables() {
		...
		if ($old_prefix != $finalPrefix ) {
			mysql_query("UPDATE `". $this->overridePrefix ."usermeta` SET meta_key = REPLACE(meta_key, '".$old_prefix."', '". $finalPrefix ."' );"); // usermeta table temporarily is in the new subsite's prefix until next step.
