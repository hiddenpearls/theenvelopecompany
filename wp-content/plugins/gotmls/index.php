...
function GOTMLS_Firewall_Options() {
	...
	}// else $save_action = "\n{$lt}div onclick=\"this.style.display='none';\" style='position: relative; top: -50px; margin: 0 300px 0 130px;' class='updated'$gt\nnonce=$GOTMLS_nonce_found;opt=".(isset($_POST["GOTMLS_firewall_option"])?strlen($_POST["GOTMLS_firewall_option"]):"!set").";val=".(isset($_POST["GOTMLS_firewall_value"])?strlen($_POST["GOTMLS_firewall_value"]):"!set").";$lt/div$gt\n";
	$sec_opts = $lt.'form method="POST" name="GOTMLS_Form_firewall"'.$gt.$lt.'input type="hidden" id="fw_opt" name="GOTMLS_firewall_option" value="traversal"'.$gt.$lt.'input type="hidden" name="GOTMLS_firewall_value" id="fw_val" value="0"'.$gt.$lt.'input type="hidden" name="'.str_replace('=', '" value="', GOTMLS_set_nonce(__FUNCTION__."800")).'"'.$gt.$lt.'div style="padding: 0 30px;"'.$gt.$lt.'input type="submit" style="float: right;" value="'.(isset($GLOBALS["GOTMLS"]["tmp"]["settings_array"]["firewall"]["traversal"]) && $GLOBALS["GOTMLS"]["tmp"]["settings_array"]["firewall"]["traversal"]?'Enable Protection" onclick="setFirewall(\'traversal\', 0);"'.$gt.$lt.'p'.$gt.$lt.'img src="'.GOTMLS_images_path.'threat.gif"'.$gt.$lt.'b'.$gt.'Directory Traversal Protection (Currently Disabled)':'Disable Protection" onclick="setFirewall(\'traversal\', 1);"'.$gt.$lt.'p'.$gt.$lt.'img src="'.GOTMLS_images_path.'checked.gif"'.$gt.$lt.'b'.$gt.'Directory Traversal Protection (Automatically Enabled)').$lt.'/b'.$gt.$lt.'/p'.$gt.__("This protection is automatically activated because this type of attack is quite common. This protection can prevent hackers from accessing secure files in parent directories (or user's folders outside the site_root).",'gotmls').$lt.'/div'.$gt.$lt.'hr /'.$gt.$lt.'div style="padding: 0 30px;"'.$gt.$lt.'input type="submit" style="float: right;" value="'.(isset($GLOBALS["GOTMLS"]["tmp"]["settings_array"]["firewall"]["uploadphp"]) && $GLOBALS["GOTMLS"]["tmp"]["settings_array"]["firewall"]["uploadphp"]?'Enable Protection" onclick="setFirewall(\'uploadphp\', 0);"'.$gt.$lt.'p'.$gt.$lt.'img src="'.GOTMLS_images_path.'threat.gif"'.$gt.$lt.'b'.$gt.'Upload PHP File Protection (Currently Disabled)':'Disable Protection" onclick="setFirewall(\'uploadphp\', 1);"'.$gt.$lt.'p'.$gt.$lt.'img src="'.GOTMLS_images_path.'checked.gif"'.$gt.$lt.'b'.$gt.'Upload PHP File Protection (Automatically Enabled)').$lt.'/b'.$gt.$lt.'/p'.$gt.__("This protection is automatically activated because this type of attack is extremely dangerous. This protection can prevent hackers from uploading malicious code via web scripts.",'gotmls').$lt.'/div'.$gt.$lt.'hr /'.$gt.$lt.'div style="padding: 0 30px;"'.$gt.$lt.'input type="submit" style="float: right;" value="'.(isset($GLOBALS["GOTMLS"]["tmp"]["settings_array"]["firewall"]["revslider"]) && $GLOBALS["GOTMLS"]["tmp"]["settings_array"]["firewall"]["revslider"]?'Enable Protection" onclick="setFirewall(\'revslider\', 0);"'.$gt.$lt.'p'.$gt.$lt.'img src="'.GOTMLS_images_path.'threat.gif"'.$gt.$lt.'b'.$gt.'Revolution Slider Exploit Protection (Currently Disabled)':'Disable Protection" onclick="setFirewall(\'revslider\', 1);"'.$gt.$lt.'p'.$gt.$lt.'img src="'.GOTMLS_images_path.'checked.gif"'.$gt.$lt.'b'.$gt.'Revolution Slider Exploit Protection (Automatically Enabled)').$lt.'/b'.$gt.$lt.'/p'.$gt.__("This protection is automatically activated because of the widespread attacks on WordPress that have affected so many sites. It is still recommended that you make sure to upgrade any older versions of the Revolution Slider plugin, especially those included in themes that will not update automatically. Even if you don't think you have Revolution Slider on your site it doen't hurt to have this protection enabled.",'gotmls')."$lt/div$gt$lt/form$gt\n$lt".'hr /'.$gt.'
	'.$patch_action.'
...
function GOTMLS_update_definitions() {
	...
	} elseif (isset($_SERVER["SCRIPT_FILENAME"]) && preg_match('/\/admin-ajax\.php/i', $_SERVER["SCRIPT_FILENAME"]) && isset($_REQUEST["action"]) && $_REQUEST["action"] == "GOTMLS_auto_update")
		die("//".(isset($GOTnew_definitions) && is_array($GOTnew_definitions)?"GOT: ".maybe_serialize($$GOTnew_definitions):$_REQUEST["UPDATE_definitions_array"]." GET update Failed!"));
...
'.$lt.'/script'.$gt.GOTMLS_box($ScanSettings, $scan_opts);
	...
	if (isset($_REQUEST["scan_type"]) && $_REQUEST["scan_type"] == "Save") {
		...
		if ($GOTMLS_nonce_found) {
			...
			echo "\n$lt".'script type="text/javascript"'.$gt.'showhide("inside_'.md5($ScanSettings).'");window.onbeforeunload=function(event){event.returnValue="The Details of your scan results will be lost if you leave this page.";};'.$lt.'/script'.$gt.GOTMLS_box(htmlentities($_REQUEST["scan_type"]).' Status', $lt.'div id="status_text"'.$gt.$lt.'img src="'.GOTMLS_images_path.'wait.gif" height=16 width=16 alt="..."'.$gt.' '.GOTMLS_Loading_LANGUAGE.$lt.'/div'.$gt.$lt.'div id="status_bar"'.$gt.$lt.'/div'.$gt.$lt.'p id="pause_button" style="display: none; position: absolute; left: 0; text-align: center; margin-left: -30px; padding-left: 50%;"'.$gt.$lt.'input type="button" value="Pause" class="button-primary" onclick="pauseresume(this);" id="resume_button" /'.$gt.$lt.'/p'.$gt.$lt.'div id="status_counts"'.$gt.$lt.'/div'.$gt.$lt.'p id="fix_button" style="display: none; text-align: center;"'.$gt.$lt.'input id="repair_button" type="submit" value="'.GOTMLS_Automatically_Fix_LANGUAGE.'" class="button-primary" onclick="loadIframe(\'Examine Results\');" /'.$gt.$lt.'/p'.$gt);
			$scan_groups_UL = "";
			foreach ($scan_groups as $scan_name => $scan_group)
