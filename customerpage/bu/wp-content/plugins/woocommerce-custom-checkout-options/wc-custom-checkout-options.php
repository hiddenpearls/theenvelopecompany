...
/*  
Copyright (C) 2012  Terry Tsang (email : terrytsang811@gmail.com)
License: Single Site
*/
	...
	if(!class_exists('WooCommerce_Custom_Checkout_Options')){
		class WooCommerce_Custom_Checkout_Options{
			...
			public function __construct(){
				...
				$this->field_types = array('text' => __('Text',$this->tab_name), 'textarea' => __('Textarea',$this->tab_name), 'password' => __('Password',$this->tab_name), 'date' => __('Date',$this->tab_name), 'country' => __('Country',$this->tab_name), 'state' => __('State',$this->tab_name), 'select' => __('Select',$this->tab_name), 'checkbox' => __('Checkbox',$this->tab_name),'timepicker' => __('Time Picker',$this->tab_name), 'colorpicker' => __('Color Picker',$this->tab_name));
				$this->position_types = array('billing' => __('Billing',$this->tab_name), 'shipping' => __('Shipping',$this->tab_name), 'account' => __('Account',$this->tab_name), 'order' => __('Order',$this->tab_name));
			...
			public function custom_order_admin_init() {
				...
				__('Custom Order Data',$this->tab_name),
			...
			public function custom_plugin_script()
			{
				...
				wp_enqueue_script( 'custom-timepicker-script', plugins_url('/js/jquery.ui.timepicker.js', __FILE__));
				wp_enqueue_script( 'custom-colorpicker-script', plugins_url('/js/spectrum.js', __FILE__));
			...
			public function custom_plugin_stylesheet() {
				...
				//timepicker
				wp_register_style( 'custom-timepicker-stylesheet', plugins_url('/css/jquery.ui.timepicker.css', __FILE__) );
				wp_enqueue_style( 'custom-timepicker-stylesheet' );
				
				//colorpicker
				wp_register_style( 'custom-colorpicker-stylesheet', plugins_url('/css/spectrum.css', __FILE__) );
				wp_enqueue_style( 'custom-colorpicker-stylesheet' );
			}
			...
			public function init(){
				...
				if ( $this->check_woocommerce_activated() ) {
					load_plugin_textdomain( 'custom-checkout-options', false, dirname( self::$plugin_basefile ) . '/languages' );
				}
				...
				$this->checkout_fields = array(
						...
						'billing_country' 	=> array(
								...
								'required'      => true,
								'default'      	=> true,
								'options'       => array( '' => __( 'Select a country', $this->tab_name ).'&hellip;' ) + $woocommerce->countries->get_allowed_countries(),
								'order'         => '',
								'section'		=> 'billing'
						),
						'billing_state' 	=> array(
			...
			?>
				...
				<table class="wc_payment widefat" cellspacing="0">
					<tbody>
						<thead>
							<th width="1%"><?php echo _e('Enable', $this->tab_name); ?></th>
							<th><?php echo _e('Position', $this->tab_name); ?></th>
							<th><?php echo _e('Type', $this->tab_name); ?></th>
							<th><?php echo _e('Label', $this->tab_name); ?></th>
							<th><?php echo _e('Field Name', $this->tab_name); ?></th>
							<th><?php echo _e('Placeholder', $this->tab_name); ?></th>
							<th><?php echo _e('Class', $this->tab_name); ?></th>
							<th><?php echo _e('Options', $this->tab_name); ?></th>
							<th><?php echo _e('Order', $this->tab_name); ?></th>
							<th><?php echo _e('Required', $this->tab_name); ?></th>
							<th><?php echo _e('Default', $this->tab_name); ?></th>
						</thead>
							...
							<tr>
								...
								<td align="center">
									...
									<?php else: ?>
										...
										<a href="?page=woocommerce&tab=custom-checkout-options&option=<?php echo $field_name; ?>&action=delete"><input id="removeField" type="button" value="X" title="<?php _e('remove this record',$this->tab_name); ?>"></a>
									<?php endif; ?>
								</td>
							</tr>
						<?php endforeach; ?>
						<tr>
							...
							<input id="addRow" type="button" value="<?php _e('Add New Field',$this->tab_name); ?>" class="button-secondary" />
							...
							<thead>
								...
								<th><?php echo _e('Order', $this->tab_name); ?></th>
								<th><?php echo _e('Required', $this->tab_name); ?></th>
								<th><?php echo _e('Action', $this->tab_name); ?></th>
							</thead>
							<tbody class="ui-sortable">
							...
							jQuery(document).ready(function(){
								 var newRow = '\
									 <tr>\
										...
										<td><input name="<?php echo WooCommerce_Custom_Checkout_Options::$plugin_prefix; ?>newfield[label][]" type="text" value="" /></td>\
										<td>\
										...
										<td><input name="<?php echo WooCommerce_Custom_Checkout_Options::$plugin_prefix; ?>newfield[placeholder][]" type="text" value="" /></td>\
			...
			public function custom_order_details_after_order_table ( $order )
			{
				...
				if($boolNewFields)
				{
					$html .= "<h2>". __('Custom Order Data',$this->tab_name) ."</h2>";
